...
export default function HeroSection({ imageSrc = '', headerText = '', bodyText = null, scrollytelling = null }) {
	...
	return (
		<section className={`${styles.heroSection} container flex flex-col lg:flex-row lg:items-center overflow-hidden ${scrollytelling ? 'scrollytelling' : ''}`}>
			...
			<div className={`${styles.textContainerWrapper} lg:pl-32 lg:-ml-16 2xl:ml-0 ${scrollytelling ? 'container lg:max-w-none lg:pr-0 2xl:mt-52' : ''}`}>
				<h1 className='font-handwriting text-6xl xl:text-8xl 2xl:text-hero transform lg:-translate-x-8 2xl:-translate-x-40 2xl:w-[132%]'>{headerText}</h1>
				<HeroUnderlineSvg />
				{bodyText && (
					<div className='mt-8 md:max-w-prose lg:max-w-[420px]' style={{ hyphens: 'auto' }}>
