...
export default function AboutUsProfile({ imageSrc = '', name = '', bodyText = '', quote = '', desktopImageFirst = false }) {
	const ProfileImage = () => (
		<img src={imageSrc} alt={`Foto von ${name}`} className='absolute w-full h-full object-cover object-center' width='549' height='373' />
	)
	...
	return (
		<section className={`flex ${desktopImageFirst ? ' flex-row-reverse' : ''}`}>
			<div className={`flex flex-col lg:w-1/2 ${desktopImageFirst ? 'lg:ml-8' : 'lg:mr-8'}`}>
				...
				<div className={`${styles.imageContainer} mb-default lg:hidden`}>
					<ProfileImage />
				</div>
				<p className='mb-default'>{bodyText}</p>
